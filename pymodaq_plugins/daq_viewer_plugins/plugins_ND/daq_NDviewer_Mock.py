    ...
    def set_Mock_data(self):
        ...
        if self.settings.child('ROIselect', 'use_ROI').value():
            ...
            data_mock = self.settings.child('spatial_settings', 'amp').value() * (
                mylib.gauss2D(self.x_axis, self.settings.child('spatial_settings', 'x0').value(),
            ...
            try:
                self.image[self.settings.child('ROIselect', 'y0').value():self.settings.child('ROIselect',
                                                                                              'y0').value() + self.settings.child(
                    'ROIselect', 'height').value(),
                self.settings.child('ROIselect', 'x0').value():self.settings.child('ROIselect',
                                                                                   'x0').value() + self.settings.child(
                    'ROIselect', 'width').value()] = data_mock
            ...
            for indy in range(data_mock.shape[0]):
                for indx in range(data_mock.shape[1]):
                    image[indy, indx, :] = data_mock[indy, indx] * mylib.gauss1D(self.time_axis,
                              self.settings.child('temp_settings', 't0').value(),
