...
# standard import comes before ... pylint: disable=C0411
try:
...
def checkstring(cmd, answer):
    ...
    assert len(str(cmd)) > 0, 'empty cmd string'
...
class ReplyHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        rcvbuf = ''
        while True:
            rcvbuf += self.request.recv(1).decode()
            if len(rcvbuf) == 0:  # EOF received
                break
            if ord(rcvbuf[-1:]) < 32:
...
class ReplyServer(object):
    ...
    def clear(self):
        ...
        if len(self.__handler.queue):
            self.__handler.queue.clear()
        if len(self.__handler.static):
