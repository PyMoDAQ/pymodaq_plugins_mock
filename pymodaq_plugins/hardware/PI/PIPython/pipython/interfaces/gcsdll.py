...
def get_gcstranslator_dir():
    ...
    if platform in ('win32', 'cygwin'):
        ...
        try:
            ...
            gcspath = winreg.QueryValueEx(keyhandle, 'Path')[0]
        ...
        if architecture()[0] == '32bit':
            if os.path.isdir('/usr/local/PI/lib32'):
                gcspath = '/usr/local/PI/lib32'
        else:
            if os.path.isdir('/usr/local/PI/lib64'):
                gcspath = '/usr/local/PI/lib64'
    else:
        raise NotImplementedError('unsupported operating system %r' % platform)
    return gcspath
...
class GCSDll(PIGateway):
    ...
    def __prefix(self):
        ...
        if self.__dllprefix is None:
            dllname = os.path.basename(self.__dllpath)
    ...
    def RemoveStage(self, axis):
        ...
        assert isinstance(axis, basestring), 'argument for RemoveStage must be string'
