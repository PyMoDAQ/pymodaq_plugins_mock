...
def ontarget(pidevice, axes):
    ...
    if closedloopaxes:
        if pidevice.HasqONT():
            ...
            isontarget.update(dict(zip(closedloopaxes, [not x for x in ismoving])))
    if openloopaxes:
        if pidevice.HasqOSN():
            stepsleft = pidevice.qOSN(openloopaxes).values()
            isontarget.update(dict(zip(openloopaxes, [x == 0 for x in stepsleft])))
        else:
            isontarget.update(dict(zip(openloopaxes, [True] * len(openloopaxes))))
    return isontarget


def waitonready(pidevice, timeout=60, predelay=0):
...
def waitontarget(pidevice, axes=None, timeout=60, predelay=0, postdelay=0):
...
def waitonwalk(pidevice, channels, timeout=300, predelay=0, postdelay=0):
    ...
    maxtime = time() + timeout
    waitonready(pidevice, timeout, predelay)
    while not all(list(x == 0 for x in pidevice.qOSN(channels).values())):
        if time() > maxtime:
            stopall(pidevice)
...
def waitonoma(pidevice, axes=None, timeout=300, predelay=0, postdelay=0):
    ...
    while True:
        ...
        for vals in zip(*positions):
            isontarget &= 0.01 > sum([abs(vals[i] - vals[i + 1]) for i in xrange(len(vals) - 1)])
        if isontarget:
            return
        if time() > maxtime:
            stopall(pidevice)
...
def waitontrajectory(pidevice, trajectories=None, timeout=180, predelay=0, postdelay=0):
